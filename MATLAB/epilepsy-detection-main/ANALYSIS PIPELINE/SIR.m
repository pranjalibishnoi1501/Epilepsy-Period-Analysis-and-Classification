% clc
% clear
% graphs = load("theta_A_W_uni11_100ms.mat").W;
...
% 
%     % Initial conditions
%     I0 = 0.1 * ones(num_nodes, 1); % initial fraction of infected individuals
%     S0 = 0.9 * ones(num_nodes, 1); % initial fraction of susceptible individuals
%     R0 = zeros(num_nodes, 1);       % initial fraction of recovered individuals
%     y0 = [S0; I0; R0];
% 
%     % Time vector
%     tspan = [0 100];
...
% 
%     % ODE system
%     ode = @(t, y) [-beta*(A*y(num_nodes+1:2*num_nodes)).*y(1:num_nodes); 
%                     beta*(A*y(num_nodes+1:2*num_nodes)).*y(1:num_nodes) - gamma*y(num_nodes+1:2*num_nodes);
%                     gamma*y(num_nodes+1:2*num_nodes)];
% 
%     % Solve ODEs
...
%     R_final = final_states(2*num_nodes+1:end) / normalizer;
% 
...
% % Initialize arrays to store results
%average_node_infection_rate_all = zeros(L, 3, 8); % Store results for all data files

for file_index = 4:4
    % Load data file
    datafile = sprintf("theta_A_W_uni1%d_100ms.mat", file_index); % Adjust the file name accordingly
    data = load(datafile);
    graphs = data.W;
    ...
    colors = lines(8); % Change 8 to the number of data files if different
    ...
    for tt = 1:T
        ...
        R0 = zeros(L, 1);      % initial fraction of recovered individuals
        y0 = [S0; I0; R0];
        ...
        % ODE system
        ode = @(t, y) [-beta*(A*y(L+1:2*L)).*y(1:L); 
                        beta*(A*y(L+1:2*L)).*y(1:L) - gamma*y(L+1:2*L);
                        gamma*y(L+1:2*L)];
        ...
        R_final = final_states(2*L+1:end) / normalizer;
    ...
    average_node_infection_rate(:,2) = avg_infected_fraction_remaining;
    average_node_infection_rate(:,3) = avg_infected_fraction_last_250;
    %average_node_infection_rate_all(:,:,file_index) = average_node_infection_rate;
    ...
    scatter(x, y, 'o', 'MarkerEdgeColor', color);
    hold on;
